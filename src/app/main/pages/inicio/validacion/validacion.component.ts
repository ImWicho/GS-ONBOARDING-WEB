...
export class ValidacionComponent implements OnInit {
  ...
  async sendData(): Promise<void> {
    try{
      this.loading = true;
      await this.service.updateClient({id: this.id, status : 2});
      this.notiSvc.openNotification(
        {title: 'La documentación del prospecto ha sido validada', msg: 'Validado correctamente', correct:true, route: true});
        this.loading = false;
    }catch(error: any){
      this.loading = false;
